...
class Admin extends MY_Controller
{
	...
	public function addpage()
	{
		if ($_POST) {
			...
			// Slug generate
			$slug = url_title(
				$this->input->post('course_name') . '-training-in-' .
			...
			if ($this->db->insert('seo_pages', $data_arr)) {
				...
				redirect(base_url('Admin/Manageaddpage'));
			} else {
	...
	public function deletepage()
	{
		if ($this->input->post('id')) {
			...
			if ($this->db->where('id', $id)->delete('seo_pages')) {
				echo json_encode(['status' => 'success', 'msg' => 'SEO Page Deleted Successfully', 'title' => 'Success']);
			} else {
				echo json_encode(['status' => 'error', 'msg' => 'Failed to delete SEO Page', 'title' => 'Error!']);
			}
		} else {
			$id = $this->uri->segment(3);
	...
	public function ManageBanner()
	{
		if ($this->uri->segment(3)) {
			...
			$data['userdata'] = $this->db->order_by('id', 'desc')->get('banner')->result();
	...
	public function expert()
	{
		...
		if ($this->uri->segment(3)) {
			...
			$data['experdata'] = $this->db->order_by('id', 'desc')->get('teamexpert')->result();
		...
		if ($this->uri->segment(3)) {
			if ($this->uri->segment(3) == 'Add') {
				...
				if ($this->db->insert('blog', $data_arr)) {
					echo json_encode(array("status" => "success", "msg" => "Blog Successfully Added", "title" => "", "reload" => "true", "redirect" => 'false'));
				} else {
					echo json_encode(array("status" => "error", "msg" => "Something went wrong", "title" => "Something went wrong!", "reload" => "false", "redirect" => 'false'));
				}
	...
	public function Registration()
	{
		...
		if (isset($_GET['mode']) && $_GET['mode'] != '') {
			$this->db->where('student_training_location', $_GET['mode']);
		}

		// Txn Status filter
		if (isset($_GET['tnxstatus']) && $_GET['tnxstatus'] != '') {
			$tnxstatus = $_GET['tnxstatus'];
			if ($tnxstatus == 'Failed') {
				$this->db->group_start();
				$this->db->where('txn_status', 'FAILED');
				$this->db->or_where('txn_status', 'PENDING');
				$this->db->group_end();
			} else {
				$this->db->where('txn_status', $tnxstatus);
			}
		}
		...
		if ($this->uri->segment(3)) {
				...
				if ($this->form_validation->run() == false) {
					...
					if ($upload_status == 'true') {
						if ($this->db->insert('modal', $data_arr)) {
							echo json_encode(array("status" => "success", "msg" => "Modal Successfully Added", "title" => "Successfully Added!", "reload" => "true", "redirect" => 'false'));
						} else {
							echo json_encode(array("status" => "error", "msg" => "Something Went Wrong", "title" => "Something went wrong!", "reload" => "false", "redirect" => 'false'));
						}
					}
			...
			if ($this->uri->segment(3) == 'Update') {
				...
				if ($this->form_validation->run() == false) {
					...
					$filename = $userdata->image; // Default to existing image

					if (!empty($_FILES['image']['name'])) {
						$ext = pathinfo($_FILES["image"]["name"], PATHINFO_EXTENSION);
						$new_filename = md5(time()) . "_modal" . "." . $ext;
						...
						if (!$this->upload->do_upload('image')) {
							...
							// Delete old image only if upload is success
							if (file_exists('./public/uploads/modal_images/' . $filename)) {
								unlink('./public/uploads/modal_images/' . $filename);
							}
	...
	public function PlacementPartner()
	{
		if ($this->uri->segment(3)) {
			if ($this->uri->segment(3) == 'Add') {
				...
				if ($this->form_validation->run() == false) {
					echo json_encode(array("status" => "error", "msg" => "Validiatno Error", "title" => "Something went wrong!", "reload" => "false", "redirect" => 'true'));
				} else {
					$upload_status = "true";
					$ext = pathinfo($_FILES["image"]["name"], PATHINFO_EXTENSION);
	...
	{
		$data['userdata'] = $this->db->order_by('id', 'desc')->get('intern')->result();
		if ($this->uri->segment(3)) {
			if ($this->uri->segment(3) == 'Add') {
				$this->form_validation->set_rules('name', 'Name', 'required');
				$this->form_validation->set_rules('role', 'Role', 'required');
